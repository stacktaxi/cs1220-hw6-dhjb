...
void Scope::Render(wxDC &dc) {
    ...
    for(unsigned historyPos = 0; historyPos < history.size(); historyPos++) {
        ...
        if(historyPos > 0 && history[historyPos] != history[historyPos - 1]) {
            // Draw straight line indicating edge
            dc.DrawLine(xpos, true_height, xpos, false_height);
        }
        else {
            // Draw dotted line marking a tick
        ...
        if(history[historyPos] == 1) {
            dc.DrawLine(xpos, true_height, xpos + STEP_WIDTH, true_height);
        }
        else if(history[historyPos] == 0) {
            dc.DrawLine(xpos, false_height, xpos + STEP_WIDTH, false_height);
        }
        else {
            // Value is X
            dc.SetBrush(wxBrush(*wxBLACK, wxBRUSHSTYLE_BDIAGONAL_HATCH));
            dc.DrawRectangle(xpos, 5, STEP_WIDTH, GetClientSize().GetHeight() - 10);
        }
    }
